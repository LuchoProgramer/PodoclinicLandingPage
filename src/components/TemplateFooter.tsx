"use client";

import Link from "next/link";
import { 
    Phone, 
    Mail, 
    MapPin, 
    Clock,
    Facebook,
    Instagram,
    Twitter,
    Linkedin,
    Youtube
} from "lucide-react";
import { getTemplateConfig } from "../../config/template.config";

// Declaraciones globales para Analytics
declare global {
    interface Window {
        gtag?: (command: string, targetId: string, config?: any) => void;
    }
}

export function TemplateFooter() {
    const config = getTemplateConfig();
    const currentYear = new Date().getFullYear();

    // Handler para tracking de enlaces del footer
    const handleFooterLinkClick = (linkName: string) => {
        if (typeof window !== "undefined" && window.gtag) {
            window.gtag("event", "click_footer_link", {
                event_category: "seo",
                event_label: `Footer: ${linkName}`,
                value: 1
            });
        }
    };

    // Links de servicios para el footer
    const serviceLinks = config.services.slice(0, 6); // Máximo 6 servicios en footer

    // Links adicionales según la industria
    const getIndustryLinks = () => {
        switch (config.business.industry) {
            case 'medical':
                return [
                    { name: 'Blog Médico', href: '/blog' },
                    { name: 'Preguntas Frecuentes', href: '/faq' },
                    { name: 'Casos de Estudio', href: '/casos' }
                ];
            case 'business':
                return [
                    { name: 'Casos de Éxito', href: '/casos-exito' },
                    { name: 'Recursos', href: '/recursos' },
                    { name: 'Consultoría', href: '/consultoria' }
                ];
            case 'creative':
                return [
                    { name: 'Portfolio', href: '/portfolio' },
                    { name: 'Blog Creativo', href: '/blog' },
                    { name: 'Recursos', href: '/recursos' }
                ];
            default:
                return [
                    { name: 'Acerca de', href: '/nosotros' },
                    { name: 'Blog', href: '/blog' },
                    { name: 'Contacto', href: '/contacto' }
                ];
        }
    };

    const industryLinks = getIndustryLinks();

    return (
        <footer className="bg-gray-900 text-white">
            <div className="max-w-7xl mx-auto px-6 py-12">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    
                    {/* Información de la empresa */}
                    <div>
                        <h3 
                            className="text-lg font-semibold mb-4"
                            style={{ color: config.branding.primaryColor }}
                        >
                            {config.business.name}
                        </h3>
                        <p className="text-gray-300 mb-4 text-sm">
                            {config.business.description}
                        </p>
                        
                        <div className="space-y-3 text-sm">
                            <div className="flex items-center text-gray-300">
                                <Phone 
                                    className="w-4 h-4 mr-2" 
                                    style={{ color: config.branding.primaryColor }} 
                                />
                                <a 
                                    href={`tel:${config.contact.phone}`}
                                    className="hover:text-white transition-colors"
                                    onClick={() => handleFooterLinkClick("Teléfono")}
                                >
                                    {config.contact.phone}
                                </a>
                            </div>
                            
                            <div className="flex items-center text-gray-300">
                                <Mail 
                                    className="w-4 h-4 mr-2" 
                                    style={{ color: config.branding.primaryColor }} 
                                />
                                <a 
                                    href={`mailto:${config.contact.email}`}
                                    className="hover:text-white transition-colors"
                                    onClick={() => handleFooterLinkClick("Email")}
                                >
                                    {config.contact.email}
                                </a>
                            </div>
                            
                            <div className="flex items-start text-gray-300">
                                <MapPin 
                                    className="w-4 h-4 mr-2 mt-1 flex-shrink-0" 
                                    style={{ color: config.branding.primaryColor }} 
                                />
                                <span>
                                    {config.contact.address.street}, {config.contact.address.city}
                                </span>
                            </div>
                            
                            <div className="flex items-center text-gray-300">
                                <Clock 
                                    className="w-4 h-4 mr-2" 
                                    style={{ color: config.branding.primaryColor }} 
                                />
                                <span>{config.contact.hours.weekdays}</span>
                            </div>
                            
                            {config.contact.hours.saturday && (
                                <div className="flex items-center text-gray-300 ml-6">
                                    <span>{config.contact.hours.saturday}</span>
                                </div>
                            )}
                        </div>
                    </div>

                    {/* Servicios principales */}
                    <div>
                        <h3 
                            className="text-lg font-semibold mb-4"
                            style={{ color: config.branding.primaryColor }}
                        >
                            {config.business.industry === 'medical' ? 'Servicios' :
                             config.business.industry === 'business' ? 'Soluciones' :
                             'Servicios'}
                        </h3>
                        <ul className="space-y-2 text-sm">
                            {serviceLinks.map((service) => (
                                <li key={service.id}>
                                    <Link 
                                        href={service.link}
                                        className="text-gray-300 hover:text-white transition-colors"
                                        onClick={() => handleFooterLinkClick(service.title)}
                                    >
                                        {service.title}
                                    </Link>
                                </li>
                            ))}
                        </ul>
                    </div>

                    {/* Links adicionales */}
                    <div>
                        <h3 
                            className="text-lg font-semibold mb-4"
                            style={{ color: config.branding.primaryColor }}
                        >
                            {config.business.industry === 'medical' ? 'Información' :
                             config.business.industry === 'business' ? 'Recursos' :
                             'Enlaces'}
                        </h3>
                        <ul className="space-y-2 text-sm">
                            {industryLinks.map((link, index) => (
                                <li key={index}>
                                    <Link 
                                        href={link.href}
                                        className="text-gray-300 hover:text-white transition-colors"
                                        onClick={() => handleFooterLinkClick(link.name)}
                                    >
                                        {link.name}
                                    </Link>
                                </li>
                            ))}
                            
                            {config.features.faq && (
                                <li>
                                    <Link 
                                        href="/faq" 
                                        className="text-gray-300 hover:text-white transition-colors"
                                        onClick={() => handleFooterLinkClick("FAQ")}
                                    >
                                        Preguntas Frecuentes
                                    </Link>
                                </li>
                            )}
                        </ul>
                    </div>

                    {/* Redes sociales y enlaces legales */}
                    <div>
                        <h3 
                            className="text-lg font-semibold mb-4"
                            style={{ color: config.branding.primaryColor }}
                        >
                            Síguenos
                        </h3>
                        
                        {/* Redes sociales (placeholder - se pueden configurar) */}
                        <div className="flex space-x-4 mb-6">
                            <a 
                                href="#" 
                                className="text-gray-400 hover:text-white transition-colors"
                                aria-label="Facebook"
                            >
                                <Facebook className="w-5 h-5" />
                            </a>
                            <a 
                                href="#" 
                                className="text-gray-400 hover:text-white transition-colors"
                                aria-label="Instagram"
                            >
                                <Instagram className="w-5 h-5" />
                            </a>
                            {config.business.industry === 'business' && (
                                <a 
                                    href="#" 
                                    className="text-gray-400 hover:text-white transition-colors"
                                    aria-label="LinkedIn"
                                >
                                    <Linkedin className="w-5 h-5" />
                                </a>
                            )}
                        </div>

                        <ul className="space-y-2 text-sm">
                            <li>
                                <Link 
                                    href="/privacidad" 
                                    className="text-gray-300 hover:text-white transition-colors"
                                    onClick={() => handleFooterLinkClick("Privacidad")}
                                >
                                    Política de Privacidad
                                </Link>
                            </li>
                            <li>
                                <Link 
                                    href="/terminos" 
                                    className="text-gray-300 hover:text-white transition-colors"
                                    onClick={() => handleFooterLinkClick("Términos")}
                                >
                                    Términos y Condiciones
                                </Link>
                            </li>
                        </ul>
                    </div>
                </div>

                {/* Separador y copyright */}
                <div className="border-t border-gray-700 mt-8 pt-6">
                    <div className="flex flex-col md:flex-row justify-between items-center text-sm text-gray-400">
                        <div className="text-center md:text-left mb-4 md:mb-0">
                            <p>&copy; {currentYear} {config.business.name}. Todos los derechos reservados.</p>
                            <p className="mt-1">
                                {config.business.tagline} en {config.contact.address.city}
                            </p>
                        </div>
                        
                        <div className="flex items-center space-x-4">
                            <a 
                                href={`https://wa.me/${config.contact.whatsapp}`}
                                target="_blank" 
                                rel="noopener noreferrer"
                                className="hover:text-white transition-colors"
                                onClick={() => handleFooterLinkClick("WhatsApp Footer")}
                            >
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                                    <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893A11.821 11.821 0 0020.465 3.516"/>
                                </svg>
                            </a>
                            
                            <span className="text-gray-600">|</span>
                            
                            <Link
                                href="/sitemap.xml"
                                className="hover:text-white transition-colors text-xs"
                                onClick={() => handleFooterLinkClick("Sitemap")}
                            >
                                Sitemap
                            </Link>
                        </div>
                    </div>
                </div>
            </div>
        </footer>
    );
}